...
import {
  User, Mail, Phone, CreditCard, Home, MapPin, Globe,
  Building2, Shield, Save, X, Edit3, Check, AlertCircle,
  Eye, EyeOff, Lock, Calendar, Users, FileText
...
const initials = computed(() => {
  ...
  return ((parts[0]?.[0] || '') + (parts[1]?.[0] || '')).toUpperCase() || 'MA'
})
...
<template>
  <div class="space-y-5 max-w-6xl mx-auto relative min-h-[750px]">
    ...
    <template v-else-if="profile">
      ...
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="bg-[#0F2356] h-20 relative overflow-hidden">
          ...
          <!-- Commune badge -->
          <div class="absolute top-3 right-5 flex items-center gap-1.5">
            <Building2 class="w-3 h-3 text-white/30" />
            <span class="text-white/30 text-xs">{{ profile.municipality?.name ?? '—' }}</span>
          </div>
        </div>

        <div class="px-7 pb-6 relative">
          <div class="absolute -top-7 left-7">
            <div class="w-14 h-14 rounded-full bg-[#0F2356] flex items-center justify-center shadow-md"
          ...
          <div class="mt-1">
            <div class="flex items-center gap-3 flex-wrap">
              ...
              <span class="flex items-center gap-1 bg-[#F7F8FB] border border-gray-200 rounded px-2.5 py-0.5">
                <Shield class="w-3 h-3 text-[#0F2356]" />
                <span class="text-xs text-[#0F2356]">Admin Municipal</span>
              </span>
            ...
            <div class="flex items-center gap-5 mt-2 flex-wrap">
              ...
              <span class="flex items-center gap-1.5 text-gray-400 text-xs">
                <MapPin class="w-3 h-3 text-[#CC1525]" /> {{ profile.municipality?.city ?? profile.city ?? '—' }}
              </span>
              <span class="flex items-center gap-1.5 text-gray-400 text-xs">
                <Globe class="w-3 h-3" /> {{ profile.municipality?.governorate ?? '—' }}
              </span>
            </div>
          </div>
        </div>
      ...
      <div v-if="activeTab === 'profil'" class="grid lg:grid-cols-3 gap-5">
        <div class="lg:col-span-2 flex flex-col gap-4">
          ...
          <!-- Commune administrée (read-only, spécifique à l'admin municipal) -->
          <!-- <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
            <div class="flex items-center gap-2 mb-4">
              <div class="w-1 h-4 bg-[#CC1525] rounded-full" />
              <span class="text-[10px] text-[#CC1525] uppercase tracking-[0.13em]">Commune administrée</span>
            </div>
            <div class="grid md:grid-cols-2 gap-4">
              <div v-for="f in [
                { label: 'Nom de la commune', value: profile.municipality?.name ?? '—', icon: Building2 },
                { label: 'Gouvernorat', value: profile.municipality?.governorate ?? '—', icon: Globe },
                { label: 'Ville', value: profile.municipality?.city ?? '—', icon: MapPin },
                { label: 'Email officiel', value: profile.municipality?.email ?? '—', icon: Mail },
                { label: 'Téléphone', value: profile.municipality?.phone ?? '—', icon: Phone },
                { label: 'Adresse siège', value: profile.municipality?.address ?? '—', icon: Home },
              ]" :key="f.label">
                ...
                <div class="flex items-center gap-2.5 bg-[#F7F8FB] border border-gray-100 rounded-lg py-2.5 pl-3 pr-4">
                  ...
                  <span class="text-[#1A2B4A] text-sm truncate">{{ f.value }}</span>
                </div>
              </div>
            </div>
          </div> -->
        ...
        <div class="flex flex-col gap-4">
          <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            ...
            <div class="flex flex-col gap-3">
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="text-xs text-[#0F2356] bg-[#0F2356]/10 border border-[#0F2356]/10 px-2 py-0.5 rounded">Admin Municipal</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-xs text-gray-400">Commune</span>
                <span class="text-xs text-gray-500">{{ profile.municipality?.name ?? '—' }}</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-xs text-gray-400">Email vérifié</span>
          ...
          <!-- <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
            <div class="flex items-center gap-2 mb-4">
              <div class="w-1 h-4 bg-[#CC1525] rounded-full" />
              <span class="text-[10px] text-[#CC1525] uppercase tracking-[0.13em]">Périmètre d'action</span>
            </div>
            <div class="flex flex-col gap-3">
              <div class="flex items-center gap-3 p-3 bg-[#F7F8FB] rounded-lg border border-gray-100">
                <div class="w-8 h-8 rounded-lg bg-white border border-gray-200 flex items-center justify-center shrink-0">
                  <Users class="w-4 h-4 text-[#0F2356]" />
                </div>
                <div>
                  <p class="text-xs text-[#0F2356]">Gestion des agents</p>
                  <p class="text-[10px] text-gray-400 mt-0.5">{{ profile.municipality?.name ?? '—' }}</p>
                </div>
              </div>
              <div class="flex items-center gap-3 p-3 bg-[#F7F8FB] rounded-lg border border-gray-100">
                <div class="w-8 h-8 rounded-lg bg-white border border-gray-200 flex items-center justify-center shrink-0">
                  <FileText class="w-4 h-4 text-[#CC1525]" />
                </div>
                <div>
                  <p class="text-xs text-[#0F2356]">Validation des incidents</p>
                  <p class="text-[10px] text-gray-400 mt-0.5">Incidents de la commune</p>
                </div>
              </div>
            </div>
          </div> -->
      ...
      <div v-if="activeTab === 'securite'" class="grid lg:grid-cols-3 gap-5">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-5 h-fit">
          ...
          <div class="flex flex-col gap-3">
            <div v-for="item in [
              { label: 'Email vérifié', done: !!profile.email_verified_at },
              { label: 'CIN enregistré', done: !!profile.cin },
              { label: 'Téléphone renseigné', done: !!profile.phone },
              { label: 'Commune assignée', done: !!profile.municipality },
